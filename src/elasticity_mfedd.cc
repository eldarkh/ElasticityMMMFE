...
namespace dd_elasticity
{
    ...
    template <int dim>
    MixedElasticityProblemDD<dim>::MixedElasticityProblemDD (const unsigned int degree,
                ...
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                {
                    ...
                    for (unsigned int j=0; j<dofs_per_cell; ++j)
                    {
                        ...
                        local_matrix(i,j) += (scalar_product(asigma, sigma)
                                              ...
                                              + cell->diameter() * cell->diameter() * phi_i_p * phi_j_p)
                                             * fe_values.JxW(q);
                    }
    ...
    void MixedElasticityProblemDD<dim>::local_cg (const unsigned int &maxiter)
    {
        ...
        std::vector<double> alpha_side(n_faces_per_cell,0),
                alpha_side_d(n_faces_per_cell,0),
                beta_side(n_faces_per_cell,0),
        ...
        for (unsigned side=0; side<n_faces_per_cell; ++side)
            if(neighbors[side] >= 0)
            {
                // Something will be here to initialize lambda correctly, right now it is just zero
        ...
        while(iteration_counter < maxiter)
        {
            ...
            for (unsigned int side=0; side<n_faces_per_cell; ++side)
                if(neighbors[side] >= 0)
                {
                    ...
                    MPI_Send(&interface_data_send[side][0], interface_dofs[side].size(), MPI::DOUBLE, neighbors[side], this_mpi, mpi_communicator);
                    MPI_Recv(&interface_data_receive[side][0], interface_dofs[side].size(), MPI::DOUBLE, neighbors[side], neighbors[side], mpi_communicator, &mpi_status);
            ...
            pcout << "\r  ..." << cg_iteration << " iterations completed, (residual = " << fabs(alpha[0]/normB) << ")..." << std::flush;
            // Exit criterion
    ...
    void MixedElasticityProblemDD<dim>::compute_errors (const unsigned int &cycle)
    {
        ...
        double send_buf_num[6] = {s_l2_error, s_hd_error, u_l2_error, u_l2_mid_error, p_l2_error, l_int_error};
        double send_buf_den[6] = {s_l2_norm, s_hd_norm, u_l2_norm, u_l2_mid_norm, p_l2_norm, l_int_norm};
    ...
    void MixedElasticityProblemDD<dim>::output_results (const unsigned int &cycle, const unsigned int &refine, const std::string &name)
    {
        ...
        data_out_star.add_data_vector (dof_handler, solution, solution_names, data_component_interpretation);
    ...
    template <int dim>
    void MixedElasticityProblemDD<dim>::run (const unsigned int refine,
